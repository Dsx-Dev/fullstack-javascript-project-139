  ...
  return (
    <div style={{ minHeight: 'calc(100vh - 60px)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#1e1f22' }}>
      <div style={{ background: '#2b2d31', borderRadius: '12px', padding: '2.5rem', width: '100%', maxWidth: '420px', boxShadow: '0 8px 32px rgba(0,0,0,0.5)', border: '1px solid #2e2f34' }}>
        ...
        {authError && <div style={{ background: '#3d1515', border: '1px solid #ed4245', color: '#ed4245', padding: '0.6rem 1rem', borderRadius: '6px', marginBottom: '1rem', fontSize: '0.875rem' }}>{authError}</div>}
        <Formik initialValues={{ username: '', password: '' }} validationSchema={validationSchema} onSubmit={handleSubmit}>
          {({ isSubmitting }) => (
            <Form style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ color: '#b9bbbe', fontSize: '0.75rem', fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Username</label>
                <Field name="username" type="text" style={{ width: '100%', background: '#383a40', border: '1px solid #1e1f22', color: '#dcddde', padding: '10px 12px', borderRadius: '6px', fontSize: '1rem', outline: 'none', boxSizing: 'border-box' }} />
                <ErrorMessage name="username" component="div" style={{ color: '#ed4245', fontSize: '0.8rem', marginTop: '4px' }} />
              </div>
              <div>
                <label style={{ color: '#b9bbbe', fontSize: '0.75rem', fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase', display: 'block', marginBottom: '6px' }}>Password</label>
                <Field name="password" type="password" style={{ width: '100%', background: '#383a40', border: '1px solid #1e1f22', color: '#dcddde', padding: '10px 12px', borderRadius: '6px', fontSize: '1rem', outline: 'none', boxSizing: 'border-box' }} />
                <ErrorMessage name="password" component="div" style={{ color: '#ed4245', fontSize: '0.8rem', marginTop: '4px' }} />
              </div>
              <button type="submit" disabled={isSubmitting}
                style={{ background: '#4f46e5', border: 'none', color: '#fff', padding: '12px', borderRadius: '6px', fontSize: '1rem', fontWeight: '700', cursor: 'pointer', marginTop: '0.5rem', transition: 'all 0.2s' }}>
        ...
        <p style={{ color: '#96989d', textAlign: 'center', marginTop: '1.5rem', fontSize: '0.875rem' }}>
          No account?{' '}
          <Link to="/signup" style={{ color: '#7b5cf6', fontWeight: '600' }}>Sign up</Link>
        </p>
      </div>
    </div>
  );
